...
class TestScannerMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.schemaTest1 = self.load_data("tests/schemaTest1.json")
        self.schemaTest2 = self.load_data("tests/schemaTest2.json")

        # next  schemas are for testing bare array handling
        # this is a starting schema
        self.schema2startnobare = self.load_data("tests/schema2startnobare.json")

        # this adds 2 bare arrays
        self.schemaTest2bare = self.load_data("tests/schemaTest2bare.json")

        # resultant schema and objects shoulld loook like this
        self.schemaTest2nonbare = self.load_data("tests/schemaTest2nonbare.json")

        self.schemaTest4 = self.load_data("tests/schemaTest4.json")

        self.schemaTest3 = self.load_data("tests/schemaTest3.json")
        self.monsterSchema = self.load_data("tests/monsterSchema.json")
    ...
    def test_createschema2(self):
        ...
        for bqi in sorted(bqSchema2, key=lambda x: x.name):
            i = bqtools.to_dict(bqi)
            # self.pp.pprint(i)
            sa2.append(i)
        ...
        evolved = bqtools.match_and_addtoschema(
            {"string": "hello"}, expectedSchema2, logger=logger
        )
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 1"
        )
        evolved = bqtools.match_and_addtoschema(
            {"string": "hello", "integer": 52}, expectedSchema2, logger=logger
        )
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 2"
        )
        evolved = bqtools.match_and_addtoschema(
            {"string": "hello", "integer": 52, "record": {}},
            expectedSchema2,
            logger=logger,
        )
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 3"
        )
        evolved = bqtools.match_and_addtoschema(
            {"string": "hello", "integer": 52, "record": {"string2": "hello2"}},
            expectedSchema2,
            logger=logger,
        )
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 4"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 6"
        )
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 7"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 8"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 9"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 10"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 11"
        )
        evolved = bqtools.match_and_addtoschema(
            {
                "string": "hello",
        ...
        self.assertEqual(
            evolved, False, "Expected no evolve but got evolve true evolve test 12"
        )
        ...
        sa = []
        for bqi in sorted(copyoforigschema, key=lambda x: x.name):
            i = bqtools.to_dict(bqi)
            # self.pp.pprint(i)
            sa.append(i)
        ...
        self.assertEqual(
            evolved, True, "Expected evolve but got no evolve False for evolve test 13"
        )
        ...
        self.assertEqual(
            evolved, True, "Expected evolve but got no evolve False for evolve test 14"
        )
    ...
    def test_patch(self):
        ...
        for bqi in sorted(bqSchema, key=lambda x: x.name):
            i = bqtools.to_dict(bqi)
            sa.append(i)
        ...
        self.assertEqual(
            change,
            True,
            "Patch diff change result {} is not what is expected {}".format(
                change, self.pp.pformat(diff)
            ),
        )
    ...
    def test_patchbare(self):
        ...
        views = bqtools.gen_diff_views(
            "foo", "ar", "bob", bare_schema, description="A test schema"
        )
        expected_views = [
            {
                "query": """#standardSQL
SELECT
    ...
    ifnull(A3.a,"None") as `mixArraya`,
    ifnull(A3.b,"None") as `mixArrayb`,
    ifnull(A3.c,0) as `mixArrayc`,
    ifnull(A3.d,"None") as `mixArrayd`,
    ...
    ifnull(A4,"None") as stringarray
from `foo.ar.bob` as tabob
LEFT JOIN UNNEST(tabob.array) as A1
LEFT JOIN UNNEST(tabob.intarray) as A2
LEFT JOIN UNNEST(tabob.mixArray) as A3
LEFT JOIN UNNEST(tabob.stringarray) as A4
    JOIN (
      SELECT
        scantime,
...
WHERE
  (action != 0 or array_length(updatedFields) > 0)"""  # noqa: W293
            },
            {
                "query": """#standardSQL
SELECT
    ...
    CASE
    WHEN o.arrayfoo IS NULL THEN 'Added'
    WHEN l.arrayfoo IS NULL THEN 'Deleted'
    WHEN o.arrayfoo = l.arrayfoo AND o.arrayinteger3 = l.arrayinteger3 AND o.arraystring3 = l.arraystring3 AND o.intarray = l.intarray AND o.integer = l.integer AND o.mixArraya = l.mixArraya AND o.mixArrayb = l.mixArrayb AND o.mixArrayc = l.mixArrayc AND o.mixArrayd = l.mixArrayd AND o.recordappended1 = l.recordappended1 AND o.recordboolean2 = l.recordboolean2 AND o.recordfloat = l.recordfloat AND o.recordstring2 = l.recordstring2 AND o.string = l.string AND o.stringarray = l.stringarray THEN 'Same'
    ELSE 'Updated'
  END AS action,
...
ON
    ...
    AND l.stringarray=o.stringarray"""
        ...
        for i, vi in enumerate(views):
            self.assertEqual(
                vi["query"].strip(),
                expected_views[i]["query"].strip(),
                "Bare list diff  {}".format(i),
            )
...
where 1=0""",
        ...
        self.assertEqual(
            len(tranches), 4, "Unexpcted number of uncompiled view tranches"
        )
        self.assertEqual(len(tranches[0]), 2, "Unexpected number of tranche 0 views")
        self.assertEqual(
            "a.b.z" in tranches[0], True, "Unexpected view a.b.z missing from tranche 0"
        )
        self.assertEqual(
            "a.b.x" in tranches[0], True, "Unexpected view a.b.x missing from tranche 0"
        )
        self.assertEqual(len(tranches[1]), 2, "Unexpected number of tranche 1 views")
        self.assertEqual(
            "a.c.x" in tranches[1], True, "Unexpected view a.c.x missing from tranche 1"
        )
        self.assertEqual(
            "a.c.x2" in tranches[1],
            True,
            "Unexpected view a.c.x2 missing from tranche 1",
        )
        self.assertEqual(len(tranches[2]), 1, "Unexpected number of tranche 2 views")
        self.assertEqual(
            "b.c.x3" in tranches[2],
            True,
            "Unexpected view b.c.x3missing from tranche 2",
        )
        self.assertEqual(len(tranches[3]), 1, "Unexpected number of tranche 2 views")
        self.assertEqual(
            "a.c.x4" in tranches[3],
            True,
    ...
    def test_sync(self):
        ...
        logging.info(
            "Checking buckets for bqsync tests exist in right regions and with lifecycle rules..."
        )
        ...
        for src_destination in test_source_configs:
            ...
            destdatset = "bqsynctest_{}_{}".format(
                "US", src_destination["dataset_name"]
            ).replace("-", "_")
            tests.append(
                {
            ...
            destdatset = "bqsynctest_{}_{}".format(
                "EU", src_destination["dataset_name"]
            ).replace("-", "_")
            tests.append(
                {
            ...
            # set up us to eu test
            destdatset = "bqsynctest_{}_{}".format(
                "europe-west2", src_destination["dataset_name"]
            ).replace("-", "_")
            tests.append(
                {
                    "subtest": "us to eu cross region",
        ...
        for datasetname in test_destination_datasets_list:
            ...
            if bqtools.dataset_exists(bqclient, dataset_ref):
                bqclient.delete_dataset(
                    bqclient.get_dataset(dataset_ref), delete_contents=True
                )
        ...
        for test_config in test_source_configs:
            ...
            for dstconfig in test_config["tests"]:
                ...
                self.assertEqual(
                    True,
                    True,
                    "Initial Sync {} {} from bigquery-public-data..{} with {}.{}  completed".format(
                ...
                synctest.sync()
                self.assertEqual(
                    synctest.tables_avoided,
                    synctest.tables_synced,
            ...
            eutest = bqtools.MultiBQSyncCoordinator(
                [
                    "{}.{}".format(
                        destination_project, test_config["tests"][1]["destdataset"]
                    )
                ],
                [
                    "{}.{}".format(
                        destination_project, test_config["tests"][2]["destdataset"]
                    )
                ],
                srcbucket=eubucket,
                dstbucket=eu2bucket,
            ...
            eutest.sync()
            self.assertEqual(
                eutest.tables_avoided + eutest.view_avoided + eutest.routines_avoided,
    ...
    def test_gendiff(self):
        bqSchema2 = bqtools.create_schema(self.schemaTest2)
        views = bqtools.gen_diff_views(
            "foo", "ar", "bob", bqSchema2, description="A test schema"
        )
...
SELECT
    _PARTITIONTIME AS scantime,
    xxrownumbering.partRowNumber,
    ifnull(A1.test1,0) as `anotherarraytest1`,
    ifnull(A1.test2,False) as `anotherarraytest2`,
    ifnull(A2.foo,0.0) as `arrayfoo`,
    ifnull(A2.integer3,0) as `arrayinteger3`,
    ifnull(A2.string3,"None") as `arraystring3`,
    ifnull(tabob.integer,0) as `integer`,
    ifnull(tabob.record.appended1,"None") as `recordappended1`,
    ifnull(tabob.record.boolean2,False) as `recordboolean2`,
    ifnull(tabob.record.float,0.0) as `recordfloat`,
    ...
    ifnull(tabob.string,"None") as `string`
from `foo.ar.bob` as tabob
LEFT JOIN UNNEST(tabob.anotherarray) as A1
LEFT JOIN UNNEST(tabob.array) as A2
    JOIN (
      SELECT
        scantime,
        ROW_NUMBER() OVER(ORDER BY scantime) AS partRowNumber
...
SELECT
    ...
    l.scantime as laterscantime,
    CASE
    WHEN o.anotherarraytest1 IS NULL THEN 'Added'
    WHEN l.anotherarraytest1 IS NULL THEN 'Deleted'
    WHEN o.anotherarraytest1 = l.anotherarraytest1 AND o.anotherarraytest2 = l.anotherarraytest2 AND o.arrayfoo = l.arrayfoo AND o.arrayinteger3 = l.arrayinteger3 AND o.arraystring3 = l.arraystring3 AND o.integer = l.integer AND o.recordappended1 = l.recordappended1 AND o.recordboolean2 = l.recordboolean2 AND o.recordfloat = l.recordfloat AND o.recordstring2 = l.recordstring2 AND o.string = l.string THEN 'Same'
    ELSE 'Updated'
  END AS action,
...
FULL OUTER JOIN (
  ...
  WHERE
    ...
    FROM
      `foo.ar.bob` )) l
ON
    l.anotherarraytest1 = o.anotherarraytest1
...
ON
    ...
    AND l.string=o.string""",
        ...
        for vi in views:
            ...
            self.assertEqual(
                len(vi["query"].strip()),
                len(vexpected[vi["name"]]["query"].strip()),
                "Query len for view {} is not equal to what is expected\n:{}:\n:{"
                "}: diff{}".format(
                    vi["name"], vi["query"], vexpected[vi["name"]]["query"], diffstr
                ),
            )
            self.assertEqual(
                vi["query"].strip(),
                vexpected[vi["name"]]["query"].strip(),
            ...
            self.assertEqual(
                ...
                "}:".format(
                    vi["name"], vi["description"], vexpected[vi["name"]]["description"]
                ),
            )
    ...
    def test_calc_field_depth(self):
        toTest = [
            ...
            {"name": "float", "type": "FLOAT", "description": None, "mode": "NULLABLE"},
            {
                "name": "boolean",
                "type": "BOOLEAN",
                "description": None,
        ...
        depth = bqtools.calc_field_depth(toTest)
        self.assertEqual(
            depth, 2, "measured field depth should be 2 is {}".format(depth)
        )
        depth = bqtools.calc_field_depth(self.monsterSchema["schema"]["fields"])
        self.assertEqual(
            depth, 13, "measured field depth should be 13 is {}".format(depth)
        )
        ...
        self.assertEqual(
            depth, 10, "measured field depth should be 10 is {}".format(depth)
        )
